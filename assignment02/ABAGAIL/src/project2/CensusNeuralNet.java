...
//       private static final String FILENAME = "src/abalone.csv";
//    private static final String FILENAME = "src/censusIncome.csv";
    private static final String FILENAME = "src/winequality-white.csv";
//    private static String FILENAME;

    //      TODO: How many examples you have
//    private static int num_examples = 4165;
...
//    private static int num_attributes = 8;
//    private static int num_attributes = 11;
    ...
    private static final long SEED = (long) Math.random();
    ...
    private static double[] temps = {1E5, 1E8, 1E10, 1E12, 1E15};
    private static double[] coolingRates = {0.6, 0.7,
            0.8, 0.9};
...
//    private static double[] mutateRatios = {0.02, 0.04, 0.06};
    private static double[] mutateRatios = {0.06};
...
//        FILENAME = "src/" + args[0];
//        num_examples = Integer.parseInt(args[1]);
//        num_attributes = Integer.parseInt(args[2]) - 1;

        initializeInstances();
//        System.out.println("Doing Wine");
...
//         Run actual SA with best params here
        // trainIterations = 1001;
//        runSA(trainIterations, best_temp, coolingRates);
    ...
    public static void runBackprop(int trainIterations) {
        ...
        ConvergenceTrainer trainer = new ConvergenceTrainer(
                new BatchBackPropagationTrainer(trainSet, backpropNet, new SumOfSquaresError(),
                        new RPROPUpdateRule(initialLearningRate, maxLearningRate, minLearningRate)),
                backprop_threshold, 5);
        for (int i = 0; i < trainIterations; i += 5) {
            trainer.train();
            ...
            System.out.print(df.format(100 - trainError) + ',');
            System.out.println(df.format(100 - testError));
        }

        double trainError = evaluateNetwork(backpropNet, trainSet.getInstances());
        double testError = evaluateNetwork(backpropNet, testSet.getInstances());

        System.out.printf("Train Accuracy: %s%% %n", df.format(100 - trainError));
        System.out.printf("Test Accuracy: %s%% %n", df.format(100 - testError));
...
//    Convert nanoseconds to seconds
    ...
    public static void runRHC(int trainIterations) {
        ...
        try {
            //PrintStream out = new PrintStream(new FileOutputStream("rhcResults.txt"));
            //System.setOut(out);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }

//            System.out.pri˜˜ntln("===========Randomized Hill
// Climbing=========");
...
//            System.out.printf("Test Error: %s%% %n", df.format(testError));
        ...
        time_elapsed /= Math.pow(10, 9);
        System.out.printf("Time Elapsed: %s s %n", df.format(time_elapsed));
        System.out.println(df.format(time_elapsed));
    }
    ...
    public static void runSA(int trainIterations, double temp, double[] coolings) {
        try {
            //PrintStream out = new PrintStream(new FileOutputStream("saResults.txt"));
            //System.setOut(out);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
...
//        for (int j = 0; j < temps.length; j++) {
        for (int j = 0; j < coolingRates.length; j++) {
            ...
            System.out.println("== Cooling " + coolingRates[j] + " ==");
            ...
            OptimizationAlgorithm oa = new SimulatedAnnealing(1E8,
                    coolingRates[j], nnop);
...
//        System.out.printf("Error: %s%% %n", df.format(minErr));
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, int
        ...
        for(int i = 0; i < iterations; i++) {
            oa.train();
            double trainError = evaluateNetwork(network, trainSet
                    .getInstances());
            double testError = evaluateNetwork(network, testSet.getInstances());
            if (i % 5 == 0) {
                System.out.println((df.format(100 - trainError)) + ", " + (df.format(100 - testError)));
            }
        }
